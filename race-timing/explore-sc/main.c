...
#if defined(ZEN)
  #define TEST_SERVER_CORE 1
#else /* Skylake */
  #define TEST_SERVER_CORE 4
#endif /* TEST_SERVER_CORE depends on the test processor. */
...
{
  printf("Accuracy | Race Interim | Out-of-Order Detachment\n");
  printf("-------------------------------------------------\n");
...
int main()
{
  ...
  rt_server_start(TEST_SERVER_CORE, RT_MODE_SC);
  ...
  for (int ri = 1; ri <= RI_SPACE; ri++) {
    ...
    for (int od = 1; od <= OD_SPACE; od++) {
      ...
      for (int r = 0; r < REPETITIONS; r++) {
        ...
        // Race-Timing.
        result = rt_load_sc(data_address, ri, od);
